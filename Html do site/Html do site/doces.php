...
$sql = "SELECT id, descricao, preco FROM doces";
$result = $PDO->query($sql);

if ($result->rowCount() > 0) {
    while ($row = $result->fetch()) {
        array_push($doces, [$row["preco"], $row["descricao"], $row["id"]]);
    }
} else {
    echo "0 results";
}
...
{
    $sql = "SELECT id, descricao, preco FROM doces where descricao like '$pesquisa%'";
    ...
    <div class="produtos d-flex flex-wrap gap-4 p-5">
        ...
        foreach ($doces as $doce) {
            ...
            <form class='card-body d-flex flex-column align-items-start' method='POST'>
              <input type='hidden' name='id' value='$doce[2]' />
              <h5 class='card-title'>$doce[1]</h5>
              <p class='card-text'>R$ " . number_format($doce[0], 2, '.', '') . "</p>
              <input class='btn btn-primary addbutton' id='liveToastBtn' type='button' value='Adicionar ao carrinho' name='addCarrinho' onclick='add($doce[2])'>
            </form>
          </div>";
        }
        ?>
