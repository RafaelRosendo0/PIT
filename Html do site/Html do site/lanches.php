...
$sql = "SELECT id, descricao, preco FROM lanches";
$result = $PDO->query($sql);

if ($result->rowCount() > 0) {
    while ($row = $result->fetch()) {
        array_push($lanches, [$row["preco"], $row["descricao"], $row["id"]]);
    }
} else {
    echo "0 results";
}
...
if (isset($_POST["formPesquisa"])) {
    ...
    $lanches = Pesquisar($pesquisa, $PDO);
}

if (isset($_POST["addCarrinho"])) {
    $id = $_POST["id"];

    $sqlCarrinho = "SELECT descricao, preco FROM lanches WHERE id = $id";
...
{
    $sql = "SELECT id, descricao, preco FROM lanches where descricao like '$pesquisa%'";
    ...
    <div class="produtos d-flex flex-wrap gap-4 p-5">
        ...
        foreach ($lanches as $lanche) {
            ...
            <form class='card-body d-flex flex-column align-items-start' method='POST'>
              <input type='hidden' name='id' value='$lanche[2]' />
              <h5 class='card-title'>$lanche[1]</h5>
              <p class='card-text'>R$ " . number_format($lanche[0], 2, '.', '') . "</p>
              <input class='btn btn-primary addbutton' id='liveToastBtn' type='button' value='Adicionar ao carrinho' name='addCarrinho' onclick='add($lanche[2])'>
            </form>
          </div>";
        }
        ?>
